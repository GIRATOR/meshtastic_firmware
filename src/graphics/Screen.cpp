...
#include <Throttle.h>
    ...
    float Screen::estimatedHeading(double lat, double lon)
    {
        ...
        if (d < 10)
        { // haven't moved enough, keep previous heading (invalid until first real movement)
...
// some screens seem to need a kick in the pants to turn back on
    ...
    void Screen::setup()
    {
        ...
        if (((invert_val == 0x0C) || (invert_val == 0x0D)) && (contrast_val > 0) && (contrast_val < 255)){
            dispdev->setContrast(contrast_val);
            dispdev->setInvert(invert_val);
        }else{         
            contrast_val = PCF8812_CONTRAST;
            invert_val = PCF8812_INVERT;
            uiconfig.screen_rgb_color = ((uint32_t)0x00 << 24) | ((uint32_t)0x00 << 16) | ((uint32_t)invert_val << 8) | (uint32_t)contrast_val;
        }
#endif
